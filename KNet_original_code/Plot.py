...
class Plot:
    ...
    def NNPlot_epochs(self, N_Epochs_plt, MSE_KF_dB_avg,
                      MSE_test_dB_avg, MSE_cv_dB_epoch, MSE_train_dB_epoch, loss_type):

        # File Name
        fileName = self.folderName + f'loss_plot_{loss_type}'
        ...
        plt.plot(x_plt, y_plt1, KColor[0], label=Klegend[0]+" "+loss_type)

        # CV
        y_plt2 = MSE_cv_dB_epoch[range(0, N_Epochs_plt)]
        plt.plot(x_plt, y_plt2, KColor[1], label=Klegend[1]+" "+loss_type)

        # Test
        y_plt3 = MSE_test_dB_avg * torch.ones(N_Epochs_plt)
        plt.plot(x_plt, y_plt3, KColor[2], label=Klegend[2]+" "+loss_type)
        ...
        plt.title(f"{self.modelName}: {loss_type} MSE Loss  [dB] - per Epoch", fontsize=fontSize)
        plt.savefig(fileName)


    def NNPlot_Hist(self, MSE_KN_linear_arr, MSE_KF_data_linear_arr=None):
    ...
    def NNPlot_test(self, MSE_KF_linear_arr, MSE_KF_linear_avg, MSE_KF_dB_avg,
